...
$.extend($.fn.datagrid.defaults.editors, { timespinner: {//timespinner就是你要自定义editor的名称 
init: function(container, options){ var input = $('<input class="easyuitimespinner">').appendTo(container); return input.timespinner({ formatter: function(value){return value;}}); }, 
getValue: function(target){return getTimes($(target).parent().find('.validatebox-text').val()); }, 
setValue: function(target, value){ $(target).timespinner("setValue",formatTime(value))}, 
resize: function(target, width){var input = $(target); if ($.boxModel == true){ input.width(width - (input.outerWidth() - input.width())-41); } else { input.width(width-41); } } } });
